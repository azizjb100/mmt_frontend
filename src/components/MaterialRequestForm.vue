...
const fetchNewNumber = async () => {
  try {
    ...
    const res = await axios.get(`${API_URL}/new-number?tanggal=${header.value.tanggal}`);
    header.value.nomor = res.data.newNumber;
  } catch (error) {
    console.error('Gagal mendapatkan nomor baru:', error);
    alert('Gagal mendapatkan nomor baru. Backend /api/requests mungkin belum siap.');
...
const saveData = async () => {
  if (details.value.length === 0 || details.value.every(d => !d.sku || d.qty === 0)) {
  ...
  <div class="form-container">
    ...
    <div class="header-panel">
      ...
      <div>
        ...
        <button @click="openGudangHelp">...</button> </div>
      <div>
        ...
        <tbody>
          <tr v-for="(item, index) in details" :key="index">
            ...
            <td>
              ...
              <button @click="openSpkHelp(index)">...</button> </td>
            <td>
              <input type="text" v-model="item.sku" />
              <button @click="openSkuHelp(index)">...</button> </td>
            ...
            <td>
              <button @click="deleteRow(index)" class="btn-hapus">Hapus</button> </td>
          </tr>
        </tbody>
      </table>
      <button @click="addNewRow" class="btn-tambah">Tambah Baris</button> </div>
    ...
    <div class="action-panel">
      <button @click="saveData" class="btn-simpan">Simpan</button> <button @click="refreshData" class="btn-batal">Batal</button>
    </div>
  </div>
</template>
...
.form-container { font-family: Arial, sans-serif; max-width: 1200px; margin: auto; }
.header-panel, .grid-panel, .approval-panel, .action-panel {
...
.header-panel div, .approval-panel div { margin-bottom: 10px; }
...
td input { width: 95%; box-sizing: border-box; }
...
.btn-simpan { background-color: #007bff; color: white; }
.btn-batal { background-color: #6c757d; color: white; }
.btn-tambah { background-color: #28a745; color: white; }
.btn-hapus { background-color: #dc3545; color: white; padding: 5px 8px; }
</style>