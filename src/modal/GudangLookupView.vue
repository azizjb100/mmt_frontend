...
const API_URL = 'http://localhost:8000/api/mmt/lookup/gudang'; 
...
const filteredGudangList = computed(() => {
    ...
    return gudangList.value.filter(g => 
...
const fetchGudangData = async () => {
    ...
    try {
        ...
        const errorMessage = (err.response?.data as { message?: string })?.message || 'Gagal memuat daftar Gudang.';
