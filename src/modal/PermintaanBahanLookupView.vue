 ...
 >
  <v-card class="dialog-card d-flex flex-column" style="height: 85vh;">
   ...
   <v-card-text class="pa-4 d-flex flex-column flex-grow-1">
    ...
    >
     ...
     <template #item.Keterangan="{ item }">
          ...
          {{ item.Keterangan ? item.Keterangan.substring(0, 50) + (item.Keterangan.length > 50 ? '...' : '') : '-' }}
        </span>
     </template>
     ...
     <template #no-data>
      <div class="text-center pa-4">Tidak ada data Permintaan Bahan ditemukan.</div>
     </template>
   ...
   <v-card-actions class="d-flex justify-end">
     <v-btn @click="emit('close')" color="secondary" variant="flat">Tutup</v-btn>
   </v-card-actions>
  </v-card>
 </v-dialog>
</template>
...
// --- State ---
const API_URL = 'http://localhost:8000/api/mmt/permintaan-bahan/lookup'; 
...
const fetchPermintaanData = async () => {
  ...
  try {
    ...
    const errorMessage = (err.response?.data as { message?: string })?.message || 'Gagal memuat daftar Permintaan Bahan.';
