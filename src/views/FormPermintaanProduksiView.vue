...
const createEmptyDetail = (): DetailItem => ({
  sku: '', namaBarang: '', qty: 0, satuan: '', panjang: 0, lebar: 0, keterangan: '', operator: '', spk: '', namaSPK: '', stok: 0, harga: 0, disc: 0, total: 0
});
...
const saveForm = async (saveAndNew: boolean) => {
    ...
    if (!isFormValid.value) {
        toast.error('Validasi Gagal: Gudang, Lokasi Produksi, dan minimal satu Kode/QTY harus diisi.');
...
const closeForm = () => { router.push({ name: 'PermintaanProsuksiBrowse' }); }; 
const printSlip = () => { toast.info(`TODO: Mencetak Slip untuk Nomor: ${formData.nomor}`); };
...
const openSPKSearch = (index: number) => { currentDetailIndex.value = index; isSPKModalVisible.value = true; };
...
const openBahanSearch = (index: number) => { currentDetailIndex.value = index; isBahanModalVisible.value = true; };
const closeBahanModal = () => { isBahanModalVisible.value = false; };
...
<v-btn size="small" color="primary" @click="saveForm(false)" :loading="isSaving" :disabled="isSaving || !isFormValid">
<v-icon start>mdi-check-circle</v-icon> Simpan
</v-btn>
<v-btn size="small" color="success" @click="saveForm(true)" :disabled="isSaving || !isFormValid">
...
<v-row dense>
<v-col cols="12">
<v-text-field label="Nomor Transaksi" v-model="formData.nomor" readonly filled
density="compact" hide-details />
</v-col>
<v-col cols="6">
<v-text-field label="Tanggal" v-model="formData.tanggal" type="date"
variant="outlined" density="compact" hide-details />
</v-col>
<v-col cols="6">
<v-text-field label="Gudang Asal" v-model="formData.gudangKode"
...
<v-col cols="12">
<v-text-field label="Nama Gudang" v-model="formData.gudangNama" readonly filled
density="compact" hide-details />
</v-col>
<v-col cols="12">
<v-text-field label="Lokasi Produksi" v-model="formData.lokasiProduksiKode"
...
<v-col cols="12">
<v-text-field label="Nama Lokasi Produksi" v-model="formData.lokasiProduksiNama" readonly filled
density="compact" hide-details />
</v-col>
<v-col cols="12">
<v-textarea label="Keterangan Header" v-model="formData.keteranganHeader" rows="2"
variant="outlined" density="compact" hide-details />
</v-col>
<v-col cols="12">
<v-text-field label="Total QTY Minta" :model-value="calculatedTotal" readonly filled
...
<v-card class="desktop-form-section flex-grow-1 d-flex flex-column" flat>
<v-card-title class="text-subtitle-1">Detail Permintaan Bahan</v-card-title>
<v-card-text class="pa-0 flex-grow-1">
<div class="detail-table-wrapper">
<v-data-table :headers="detailHeaders" :items="formData.detail" :items-per-page="-1"
class="elevation-0 detail-entry-table" hide-default-footer>
...
<v-text-field v-model="item.sku" @click:append-inner="openBahanSearch(index)"
append-inner-icon="mdi-magnify" variant="underlined" density="compact" hide-details style="cursor: pointer;" />
</template>

<template #[`item.namaBarang`]="{ item }">
<v-text-field v-model="item.namaBarang" readonly filled density="compact" hide-details />
</template>

<template #[`item.qty`]="{ item }">
<v-text-field v-model.number="item.qty" type="number" min="0"
variant="underlined" density="compact" hide-details class="text-end" />
</template>

<template #[`item.satuan`]="{ item }">
<v-text-field v-model="item.satuan" readonly filled density="compact" hide-details />
</template>

<template #[`item.panjang`]="{ item }">
<v-text-field v-model.number="item.panjang" type="number" min="0" readonly filled density="compact" hide-details class="text-end" />
</template>

<template #[`item.lebar`]="{ item }">
<v-text-field v-model.number="item.lebar" type="number" min="0" readonly filled density="compact" hide-details class="text-end" />
</template>

<template #[`item.operator`]="{ item }">
<v-text-field v-model="item.operator" variant="underlined" density="compact" hide-details />
</template>

<template #[`item.spk`]="{ item, index }">
<v-text-field v-model="item.spk" @click:append-inner="openSPKSearch(index)" append-inner-icon="mdi-magnify" variant="underlined" density="compact" hide-details style="cursor: pointer;" />
</template>

<template #[`item.stok`]="{ item }">
<v-text-field :model-value="item.stok" readonly filled density="compact" hide-details class="text-end" />
</template>
...
@click="removeDetail(index)" :disabled="formData.detail.length === 1"
title="Hapus baris" />
</template>
...
<v-row justify="end">
<v-col cols="auto">
<v-btn size="small" color="primary" @click="addDetail"
prepend-icon="mdi-plus">Tambah Item</v-btn>
</v-col>
</v-row>
</v-container>
</template>
...
<GudangLookupModal v-if="isGudangModalVisible" :isVisible="isGudangModalVisible" @close="() => isGudangModalVisible = false" @select="handleGudangAsalSelect" />
<GudangLookupModal v-if="isLokasiProdModalVisible" :isVisible="isLokasiProdModalVisible" @close="() => isLokasiProdModalVisible = false" @select="handleGudangProdSelect" />
<MasterBahanModal  :isVisible="isBahanModalVisible" mode="mmt"  @close="closeBahanModal" @select="handleBahanSelect" />
<SPKLookupModal v-if="isSPKModalVisible" :isVisible="isSPKModalVisible" @close="() => isSPKModalVisible = false" @select="handleSPKSelect" />
</PageLayout>
</template>
