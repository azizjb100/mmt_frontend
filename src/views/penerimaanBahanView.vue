...
const detailHeaders = [
    ...
    { title: 'Jml Terima', key: 'Jumlah_Terima', minWidth: '100px', align: 'end' },
...
const handleNewEdit = (mode) => {
    if (mode === 'new') {
        ...
        router.push({ name: 'PenerimaanBahanEdit', params: { nomor: selectedNomor.value } });
    }
};
...
const handleDelete = async () => {
    ...
    try {
        ...
        const err = error;
        alert(`Gagal Hapus: ${err.response?.data?.message ?? err.message ?? 'Terjadi kesalahan.'}`);
    }
};
