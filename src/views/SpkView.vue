...
const API_SPK_BROWSE = "/mmt/spk/browse";
...
const filteredMasterData = computed(() => {
  return masterData.value.filter((item) => {
    ...
    const matchesSpk =
      !filterSpkInput.value ||
      item.SPK.toLowerCase().includes(
        filterSpkInput.value.trim().toLowerCase(),
      );
...
const fetchData = async () => {
  ...
  expanded.value = [];
  try {
    const res = await api.get(API_SPK_BROWSE, {
      params: {
...
const handleExpandUpdate = async (expandedKeys: any[]) => {
  ...
  const spkNomor = typeof lastItem === "object" ? lastItem.SPK : lastItem;
  if (!spkNomor || details.value[spkNomor]) return;

  loadingDetails.value.add(spkNomor);
  try {
    const res = await api.get(
      `/mmt/spk/detail-size/${encodeURIComponent(spkNomor)}`,
    );
...
const handleRowClick = (_event: any, row: any) => {
  selected.value = selected.value.some((s) => s.SPK === row.item.SPK)
    ? []
    : [row.item];
};

const getRowProps = ({ item }: any) => ({
  class: selected.value.some((s) => s.SPK === item?.SPK) ? "row-selected" : "",
});

const handleNew = () => router.push({ name: "spk-form" });
...
<template>
  ...
  >
    ...
    <template #expanded-content="{ item }">
      <div v-if="isLoadingDetails(item.SPK)" class="text-center pa-2">
        <v-progress-circular
          indeterminate
          size="20"
          color="primary"
      ...
      <div
        v-else-if="!details[item.SPK] || details[item.SPK].length === 0"
        class="text-center pa-2 text-caption text-grey"
      >
        Tidak ada data detail ukuran untuk SPK {{ item.SPK }}
      </div>
