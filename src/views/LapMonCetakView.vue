...
// --- STATE MANAGEMENT ---
const API_URL = 'http://localhost:8000/api/mmt/monitoring-cetak/monitoring'; 
...
const filteredData = computed(() => {
    ...
    return allData.value.filter(row => {
        const noSpkMatch = row.noSpk ? row.noSpk.toLowerCase().includes(query) : false;
        const namaOrderMatch = row.namaOrder ? row.namaOrder.toLowerCase().includes(query) : false;
        const perushMatch = row.perush ? row.perush.toLowerCase().includes(query) : false;
...
const reportTotals = computed(() => {
    return filteredData.value.reduce((acc, row) => {
        ...
        order_meter: 0, pcs: 0, jmlcetak: 0, cetak_luar: 0, jmlkurang: 0, 
        mt01: 0, mt02: 0, mt03: 0, mt04: 0, mt05: 0, 
        jmt01: 0, jmt02: 0, jmt03: 0, jmt04: 0, jmt05: 0, 
        clmtjumlah: 0,
    });
});
...
const fetchReport = async () => {
    ...
    try {
        ...
        console.log(`Laporan Monitoring Cetak dimuat: ${startDate.value} s/d ${endDate.value}`);
        ...
        allData.value = [
            { noSpk: 'SPK/MT/2401/001', tglSpk: '2025-11-01', tglLhk: '2025-11-01', deadline: '2025-11-15', perush: 'PT KAIN MAJU', namaOrder: 'Bahan Kaos Biru', panjang: 1.5, lebar: 2.0, order_meter: 3000, pcs: 1200, jenis: 'V-TEX', jmlcetak: 1000, cetak_luar: 0, jmlkurang: 200, mt01: 500, mt02: 500, mt03: 0, mt04: 0, mt05: 0, jmt01: 1500, jmt02: 1500, jmt03: 0, jmt04: 0, jmt05: 0, clmtjumlah: 1000 },
            { noSpk: 'SPK/MT/2401/002', tglSpk: '2025-11-05', tglLhk: '2025-11-05', deadline: '2025-11-20', perush: 'CV PRINT JAYA', namaOrder: 'Spanduk Promosi', panjang: 10, lebar: 1.2, order_meter: 1200, pcs: 100, jenis: 'FLEXI', jmlcetak: 0, cetak_luar: 0, jmlkurang: 1200, mt01: 0, mt02: 0, mt03: 0, mt04: 0, mt05: 0, jmt01: 0, jmt02: 0, jmt03: 0, jmt04: 0, jmt05: 0, clmtjumlah: 0 },
            { noSpk: 'SPK/MT/2401/003', tglSpk: '2025-11-10', tglLhk: '2025-11-10', deadline: '2025-11-25', perush: 'PT TEKSTIL INDAH', namaOrder: 'Bahan Batik Digital', panjang: 1.0, lebar: 1.5, order_meter: 5000, pcs: 2500, jenis: 'B.DIGITAL', jmlcetak: 4500, cetak_luar: 500, jmlkurang: 0, mt01: 1500, mt02: 1500, mt03: 1500, mt04: 0, mt05: 0, jmt01: 2250, jmt02: 2250, jmt03: 2250, jmt04: 0, jmt05: 0, clmtjumlah: 4500 },
            ...Array(20).fill(null).map((_, i) => ({
                noSpk: `SPK/DM/2411/${100 + i}`, tglSpk: '2025-11-12', tglLhk: '2025-11-13', deadline: '2025-11-20', perush: `CV Demo ${i}`, namaOrder: `Banner Test ${i}`, panjang: 2.0, lebar: 1.0, order_meter: 20, pcs: 5, jenis: 'FLEXI', jmlcetak: 5, cetak_luar: 0, jmlkurang: 0, mt01: 5, mt02: 0, mt03: 0, mt04: 0, mt05: 0, jmt01: 10, jmt02: 0, jmt03: 0, jmt04: 0, jmt05: 0, clmtjumlah: 5 
            }))
        ].map(row => ({...row, tglLhk: row.tglSpk })); // Map tglSpk ke tglLhk
        console.warn('Memuat data demonstrasi untuk Laporan Monitoring Cetak. Pastikan Express Server berjalan.');
    }
...
const exportToExcel = () => {
    console.info('Simulasi Ekspor Laporan Monitoring Cetak: Siap mengekspor ' + filteredData.value.length + ' baris data.');
};
        ...
        <div class="panel header-panel"> 
            ...
            <div class="header-top">
                ...
                <div class="filter-controls">
                    <label>Periode:</label>
                    <input type="date" v-model="startDate" title="Tanggal Mulai" class="date-input" />
                    <span>s.d</span>
                    <input type="date" v-model="endDate" title="Tanggal Akhir" class="date-input" />

                    <button @click="fetchReport" class="btn btn-primary" :disabled="loading.report">
            ...
            <div class="header-bottom">
                <div class="search-group">
                    ...
                    <input type="text" v-model="searchQuery" placeholder="Cari No. SPK, Nama Order, atau Perusahaan..." />
                </div>
            
                <button class="btn btn-secondary" @click="exportToExcel">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 d 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5"/>
                        <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L6.354 8.146a.5.5 0 1 0-.708.708z"/>
                    </svg>
                    Export
                </button>
            </div>
        ...
        <div class="panel table-panel">
            <div class="table-wrapper">
                <table style="border-collapse: collapse;">
                    ...
                    <tbody class="table-body">
                        ...
                        <tr v-for="row in paginatedData" :key="row.noSpk"
                            :class="{'row-highlight': row.jmlcetak === 0 && row.cetak_luar === 0}">
                            ...
                            <!-- TOTAL QTY (PCS) -->
                            <td class="text-right">{{ formatNumber(row.jmlcetak + row.cetak_luar, 0) }}</td>
                    ...
                    <tfoot class="table-footer" v-if="filteredData.length > 0">
                        <tr>
                            ...
                            <!-- METER -->
                            <th class="text-right">{{ formatNumber(reportTotals.order_meter, 2) }}</th>
                            <!-- JENIS -->
                            <th></th>

                            <!-- QTY CETAK (PCS) -->
                            <th class="text-right">{{ formatNumber(reportTotals.mt01, 0) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.mt02, 0) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.mt03, 0) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.mt04, 0) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.mt05, 0) }}</th>

                            <!-- TOTAL QTY (PCS) -->
                            <th class="text-right">{{ formatNumber(reportTotals.jmlcetak + reportTotals.cetak_luar, 0) }}</th>
                            
                            <!-- VOLUME METER (M) -->
                            <th class="text-right">{{ formatNumber(reportTotals.jmt01, 2) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.jmt02, 2) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.jmt03, 2) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.jmt04, 2) }}</th>
                            <th class="text-right">{{ formatNumber(reportTotals.jmt05, 2) }}</th>
                            
                            <!-- KURANG (PCS) -->
                            <th class="text-right">{{ formatNumber(reportTotals.jmlkurang, 0) }}</th>
                        </tr>
                    </tfoot>
                </table>
            </div>
            ...
            <div class="pagination-controls" v-if="filteredData.length > 0">
                ...
                <div class="pagination-nav">
                    <button @click="prevPage" :disabled="currentPage === 1 || itemsPerPage === -1" class="btn-page">
                        &laquo; Prev
                    </button>
                    ...
                    <button @click="nextPage" :disabled="currentPage === totalPages || itemsPerPage === -1" class="btn-page">
                        Next &raquo;
                    </button>
                </div>
