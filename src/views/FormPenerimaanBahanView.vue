<template>
                ...
                <v-card class="desktop-form-section" flat>
                    ...
                    <v-card-text>
                        <v-row dense>
                            ...
                            <v-col cols="6">
                                <v-text-field label="Gudang" :model-value="`${formData.gudangKode} - ${formData.gudang}`"
                                    readonly filled density="compact" hide-details />
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Nomor PO/Minta" v-model="formData.noPermintaan" readonly
                                    @click="openPOLookup" variant="outlined" density="compact" hide-details
                                    append-inner-icon="mdi-magnify" style="cursor: pointer;" />
                            </v-col>
                            <v-col cols="5">
                                <v-text-field label="Kode Supplier" v-model="formData.supplierKode" readonly
                                    @click="openSupplierSearch" variant="outlined" density="compact" hide-details
                                    append-inner-icon="mdi-magnify" style="cursor: pointer;" />
                            </v-col>
                            <v-col cols="7">
                                <v-text-field label="Nama Supplier" v-model="formData.supplier" readonly filled
                                    density="compact" hide-details />
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Alamat" v-model="formData.alamat" readonly filled
                                    density="compact" hide-details />
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Telepon" v-model="formData.telp" readonly filled
                                    density="compact" hide-details />
                            </v-col>
                            <v-col cols="12">
                                <v-textarea label="Keterangan" v-model="formData.keterangan" rows="2"
                                    variant="outlined" density="compact" hide-details />
                            </v-col>
                        </v-row>
                    </v-card-text>
                ...
                <v-card class="desktop-form-section flex-grow-1 d-flex flex-column" flat>
                    ...
                    <v-card-text class="pa-0 flex-grow-1">
                        <div class="detail-table-wrapper">
                            <v-data-table :headers="detailHeaders" :items="formData.details" :items-per-page="-1"
    ...
    />
                                ...
                                <template #[`item.qtyTerima`]="{ item }">
                                    <v-text-field v-model.number="item.qtyTerima" type="number" min="0"
                                        variant="underlined" density="compact" hide-details class="text-end" />
                                </template>

                                <template #[`item.panjang`]="{ item }">
                                    <v-text-field v-model.number="item.panjang" type="number" min="0"
                                        variant="underlined" density="compact" hide-details class="text-end" />
                                </template>

                                <template #[`item.lebar`]="{ item }">
                                    <v-text-field v-model.number="item.lebar" type="number" min="0"
                                        variant="underlined" density="compact" hide-details class="text-end" />
                                </template>
                                
                                <template #[`item.qtyPO`]="{ item }">
                                    <v-text-field :model-value="item.qtyPO" readonly filled density="compact" hide-details class="text-end" />
                                </template>
                                
                                <template #[`item.namaBahan`]="{ item }">
                                    <v-text-field :model-value="item.namaBahan" readonly filled density="compact" hide-details />
                                </template>
                                
                                <template #[`item.satuan`]="{ item }">
                                    <v-text-field :model-value="item.satuan" readonly filled density="compact" hide-details />
                                </template>
                                ...
                                <template #[`item.actions`]="{ index }">
                                    <v-btn icon="mdi-delete" size="x-small" variant="text" color="error"
                                        @click="removeDetail(index)" :disabled="formData.details.length === 1"
                                        title="Hapus baris" />
                                </template>
...
const API_BASE_URL = 'http://localhost:8000/api/mmt/penerimaan-bahan';
const API_SUPPLIER_DETAIL = 'http://localhost:8000/api/supplier'; 
const API_MASTER_BAHAN_DETAIL_SINGLE = 'http://localhost:8000/api/master/bahan/mmt'; 
const API_PO_LOOKUP_DETAIL = 'http://localhost:8000/api/mmt/penerimaan-bahan/po/lookup'; 
...
const createEmptyDetail = (): DetailItem => ({
    kode: '', namaBahan: '', qtyPO: 0, qtyTerima: 0, panjang: 0, lebar: 0, satuan: '', keterangan: ''
});
...
const detailHeaders = [
  ...
  { title: 'Qty Terima', key: 'qtyTerima', width: '10%', align: 'end' as const },
...
const handleSupplierSelect = async (selectedSupplier: { Kode: string }) => {
  ...
  try {
    ...
    const response = await axios.get<{ data: SupplierData }>(`${API_SUPPLIER_DETAIL}/${selectedSupplier.Kode}`);
...
const handleManualBahanInput = async (item: DetailItem, index: number) => {
    ...
    const isDuplicate = formData.details.some(d => d.kode === kode && d !== item);
    if (isDuplicate) {
    ...
    try {
        const response = await axios.get<{ data: MasterBahan }>(`${API_MASTER_BAHAN_DETAIL_SINGLE}/${kode}`);
...
const handlePOSelect = async (poHeader: PoHeader) => {
    ...
    try {
        ...
        const response = await axios.get<{ data: PoLookupData }>(`${API_PO_LOOKUP_DETAIL}/${poHeader.Nomor}`);
        ...
        toast.error(`Gagal memuat detail PO: ${err.response?.data?.message || err.message}`);
    }
};
...
const addDetail = () => { formData.details.push(createEmptyDetail()); };
...
const saveForm = async (saveAndNew: boolean) => {
  ...
  if (!formData.supplierKode || validDetails.length === 0 || !validDetails.every(d => d.qtyTerima > 0)) {
    toast.error('Mohon lengkapi Supplier dan minimal satu Kode/Qty Terima yang valid.');
...
const printSlip = () => { toast.info(`Mencetak Slip untuk Nomor: ${formData.nomor}`); };
