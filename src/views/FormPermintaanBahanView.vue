...
const approveData = async (type: "SPV" | "MANAGER") => {
    if (
        ...
        toast.warning(
            "Manager tidak dapat melakukan ACC sebelum SPV menyetujui.",
        );
        return;
    }
    ...
    try {
        ...
        const msg =
            error.response?.data?.message || `Gagal melakukan ACC ${type}`;
        toast.error(msg);
    } finally {
...
const saveForm = async (saveAndNew: boolean) => {
    ...
    try {
        const payload = {
            ...
            AccSpvUser:
                formData.accSpv && formData.accSpv !== "-"
                    ? formData.accSpv
                    : null,
    ...
    <PageLayout title="Form Permintaan Barang" icon="mdi-basket-fill">
        <template #header-actions>
            ...
            <v-divider
                ...
                v-if="
                    isEditMode &&
                    (currentUserKode === 'EKAMMT' ||
                        currentUserKode === 'FADLY')
                "
            ></v-divider>
                        ...
                        <v-row dense>
                            ...
                            <v-col cols="6">
                                <v-text-field
                                    ...
                                    @click="
                                        !isLocked &&
                                        (isGudangModalVisible = true)
                                    "
                                    :append-inner-icon="
                                        isLocked ? '' : 'mdi-magnify'
                                    "
                                    readonly
                                    density="compact"
                                    variant="outlined"
                            ...
                            <v-col cols="12" class="pt-1">
                                <v-select
                                    ...
                                    :items="['Bahan Baku', 'Sparepart', 'ATK']"
                                    variant="outlined"
                                    density="compact"
                                    hide-details
                                    :readonly="isLocked"
                            ...
                            <v-col cols="12" class="mt-4 pt-4 border-t">
                                <div
                                    class="text-caption font-weight-bold text-grey mb-2"
                                >
                                    STATUS APPROVAL
                                </div>
                                <v-row dense>
