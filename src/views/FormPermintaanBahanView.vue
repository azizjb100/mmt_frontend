...
const API_URL = 'http://localhost:8000/api/mmt/permintaan-bahan';
const API_MASTER_BAHAN = 'http://localhost:8000/api/master/bahan/mmt';
...
const createEmptyDetail = (): DetailItem => ({
 sku: '', namaBarang: '', qty: 0, satuan: '', Panjang: 0, Lebar: 0, keterangan: '', operator: '', spk: '', namaSPK: '', harga: 0, disc: 0, total: 0
});
...
const formData = reactive<FormDataState>({
 ...
 keteranganHeader: '',
 reqAcc: 'T', reqAccUser: '', acc: 'T', accUser: '', 
...
const detailSubTotal = computed(() => {
  ...
  return formData.detail.map(d => (d.qty || 0) * (d.harga || 0) * (1 - (d.disc || 0) / 100));
});
...
const detailHeaders = [
 { title: 'No', key: 'index', width: '5%', sortable: false, align: 'center' as const },
...
const saveForm = async (saveAndNew: boolean) => { 
  if (!isFormValid.value) {
    toast.error('Data Header (Gudang/Tanggal) atau Detail (Kode/QTY) belum lengkap.');
    return;
  }
  isSaving.value = true;
  try {
...
const closeForm = () => { router.push({ name: 'PermintaanBahanBrowse' }); }; 
const printSlip = () => { toast.info(`TODO: Mencetak Slip untuk Nomor: ${formData.nomor}`); };
const handleReqAccChange = (value: 'Y' | 'T') => { formData.reqAccUser = value === 'Y' ? userLogin : ''; };
const handleAccChange = (value: 'Y' | 'T') => { formData.accUser = value === 'Y' ? userLogin : ''; };
...
const loaddataall = async (nomor: string) => {
 ...
 try {
  ...
  formData.detail = apiData.Detail.map((d: any) => ({
   ...
   spk: d.Nomor_SPK || '',
   namaSPK: d.spk_nama || '',
   harga: 0, disc: 0, total: 0 // Kosongkan atau ambil dari DB jika ada
  }));
  ...
  toast.error((err.response?.data as { message?: string })?.message || 'Gagal memuat data transaksi.');
  console.error(err);
 } finally {
  isSaving.value = false;
 }
...
<v-btn size="x-small" color="primary" @click="saveForm(false)" :loading="isSaving" :disabled="isSaving || !isFormValid">
<v-icon start>mdi-check-circle</v-icon> Simpan
</v-btn>
<v-btn size="x-small" color="success" @click="saveForm(true)" :disabled="isSaving || !isFormValid">
<v-icon start>mdi-content-save-all</v-icon> Simpan & Baru
</v-btn>
<v-btn size="x-small" color="teal" @click="printSlip" :disabled="!formData.nomor || formData.nomor === 'AUTO'">
...
<v-text-field v-model="item.namaBarang" variant="plain" readonly filled density="compact" hide-details />
</template>
...
<v-text-field :model-value="item.Lebar" variant="plain" readonly filled density="compact" hide-details class="text-end" />
</template>
<template #[`item.Panjang`]="{ item }">
<v-text-field :model-value="item.Panjang" variant="plain" readonly filled density="compact" hide-details class="text-end" />
</template>

<template #[`item.satuan`]="{ item }">
<v-text-field v-model="item.satuan" variant="plain" readonly filled density="compact" hide-details />
</template>

<template #[`item.keterangan`]="{ item }">
<v-text-field v-model="item.keterangan" density="compact" hide-details />
</template>

<template #[`item.actions`]="{ index }">
<v-btn icon="mdi-delete" size="x-x-small" variant="text" color="error"
@click="removeDetail(index)" :disabled="formData.detail.length === 1 && !formData.detail[0].sku"
title="Hapus baris" />
</template>
