...
<template>
  <div class="po-print-container">
    <div v-if="isLoading" class="text-center loading-message">Memuat data PO...</div>
    ...
    <div v-if="printData" class="po-page">
      <header class="po-header">
        ...
        <div class="po-title-section">
          ...
          <div class="po-meta">
            <div><span class="meta-label">Date</span>: {{ printData.Header.Tanggal }}</div>
            <div><span class="meta-label">P.O. Number</span>: {{ printData.Header.Nomor }}</div>
      ...
      <div class="vendor-section">
        ...
        <div class="vendor-details">
          ...
          <div class="vendor-address">{{ printData.Header.AlamatSupplier || 'N/A' }}</div>
          <div class="vendor-city">{{ printData.Header.KotaSupplier || 'SOLO' }}</div>
        </div>
      </div>
        ...
        <table class="items-table">
          ...
          <tbody>
            <tr v-for="(item, index) in printData.Detail" :key="index">
              ...
              <td class="text-right">{{ formatRupiah(item.Quantity, 2) }} {{ item.Satuan }}</td>
              <td class="text-right">{{ formatRupiah(item.UnitPrice) }}</td>
              <td class="text-right">{{ formatRupiah(item.Total) }}</td>
            </tr>
            <tr v-for="i in Math.max(0, 10 - printData.Detail.length)" :key="'empty-' + i" class="empty-row">
              <td>&nbsp;</td>
              <td></td>
              <td></td>
              <td></td>
        ...
        <div class="total-summary-wrapper">
          <div class="total-summary-box">
            <div class="total-row">
              ...
              <span class="amount">{{ formatRupiah(printData.Header.SubTotal) }}</span>
            </div>
            <div v-if="printData.Header.IsPpn === 1" class="total-row tax">
              <span class="label">PPN {{ printData.Header.PpnRate }}%</span>
              <span class="amount">{{ formatRupiah(printData.Header.TotalPpn) }}</span>
          </div>
          <div class="total-row grand-total-line">
            <span class="label">TOTAL</span>
            <span class="amount">{{ formatRupiah(printData.Header.GrandTotal) }}</span>
          </div>
        </div>
        </div>
...
.company-section { width: 50%; line-height: 1.3; }
.company-name { font-size: 12pt; font-weight: 900; margin-top: 0; margin-bottom: 2px; }
.company-section address { font-style: normal; font-size: 9pt; }
.po-title-section { width: 45%; text-align: right; }
.po-title-section h2 { color: #000080; font-size: 18pt; font-weight: 900; margin: 0 0 10px 0; }
.po-meta { line-height: 1.5; font-size: 9pt; }
.po-meta .meta-label { font-weight: bold; display: inline-block; width: 90px; }

.vendor-section { margin: 15px 0; border: 1px solid #000080; line-height: 1.4; }
.vendor-header { background-color: #000080; color: white; font-weight: bold; padding: 3px 10px; font-size: 10pt; }
.vendor-details { padding: 5px 10px; }

/* =================================================================
   ITEMS TABLE (Tidak berubah)
   ================================================================= */
.items-table { width: 100%; border-collapse: collapse; }
.items-table th, .items-table td { border: 1px solid black; padding: 6px 8px; vertical-align: top; height: 15pt; }
...
.items-table td.text-right { font-size: 10pt; }
.items-table .empty-row td { height: 15pt; }
...
.notes-section p { padding: 5px 10px; margin: 0; font-size: 9pt; }
.delivery-address { padding-top: 5px; font-style: italic; font-size: 9pt; }
...
.signer-role { font-size: 9pt; margin-top: -5px; }
.signature-line { border-top: 1px solid black; margin-top: 35px; }
