...
const api = axios;
const API_BASE_URL = 'http://localhost:8000/api/mmt/operator';
...
const handleEdit = () => {
 ...
 router.push({ name: 'OperatorMmtEdit', params: { kode: selectedKode.value } });
};
...
const handleDelete = async () => {
 ...
 if (confirm(`Yakin ingin hapus Operator ${selectedKode.value} (${selectedRow.value?.Nama})?`)) {
  try {
