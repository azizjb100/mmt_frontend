   ...
   <div class="table-container">
    ...
    >
     ...
     <template #item.Lengkap="{ item }">
      <v-chip size="x-x-small" :color="item.Lengkap === 'Y' ? 'success' : 'error'">
       {{ item.Lengkap === 'Y' ? 'YA' : 'TIDAK' }}
          ...
          <template #item.cetak_meter="{ item }">
             <span :class="getRowTextColor(item)">{{ formatMeter(Number(item.cetak_meter || 0)) }}</span>
          </template>
...
const handleNewEdit = (mode: 'new' | 'edit') => {
  if (mode === 'new') {
    ...
    router.push({ name: 'PermintaanProduksiEdit', params: { nomor: selectedNomor.value } });
  }
};
...
const resizeTable = (tableSelector: string) => {
    ...
    headers.forEach((header, index) => {
        ...
        if (header.classList.contains('v-data-table__th--select') || header.classList.contains('v-data-table__th--group')) {
            return;
        }
        ...
        resizer.addEventListener('mousedown', (e: MouseEvent) => {
            ...
            const columnCells = Array.from(tbody.querySelectorAll(`tr td:nth-child(${index + 1})`)) as HTMLElement[];
...
const handleDelete = async () => {
 if (!selectedRow.value) return;
 if (confirm(`Yakin ingin menghapus LHK Cetak nomor ${selectedRow.value.Nomor}?`)) {
  try {
...
const handleBahan = () => {
 ...
 router.push({ name: 'LhkCetakBahan', params: { nomor: selectedRow.value.Nomor } }); 
};
