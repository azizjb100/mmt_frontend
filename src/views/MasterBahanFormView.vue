  ...
  <div class="form-grid-container">
    ...
    <div class="left-column">
      <v-card class="desktop-form-section" flat>
          ...
          <v-row dense>
            ...
            <v-col cols="12">
              ...
              <v-text-field label="Satuan" v-model="formData.satuan" 
                        ...
                        <v-col cols="12">
                            <v-text-field label="Gramasi (Konstruksi)" v-model="formData.gramasi" 
                                variant="outlined" density="compact" hide-details />
                        </v-col>

                        <v-col cols="6">
                            <v-text-field label="Harga Beli" v-model.number="formData.hargaBeli" 
                                type="number" min="0" 
                                variant="outlined" density="compact" hide-details />
                        </v-col>
                        <v-col cols="6">
                            <v-text-field label="Harga Jual" v-model.number="formData.hargaJual" 
                                type="number" min="0" 
                                variant="outlined" density="compact" hide-details />
                        </v-col>
          </v-row>
        ...
        <v-card-text>
                    <v-row dense>
                        <v-col cols="6">
                            <v-text-field label="Panjang" v-model.number="formData.panjang" type="number" min="0"
                                variant="outlined" density="compact" hide-details />
                        </v-col>
                        <v-col cols="6">
                            <v-text-field label="Lebar" v-model.number="formData.lebar" type="number" min="0"
                                variant="outlined" density="compact" hide-details />
                        </v-col>
                        <v-col cols="12">
                            <v-checkbox label="Aktif" v-model="formData.isAktif" density="compact" hide-details />
                            <v-checkbox label="Hitung Stok" v-model="formData.isStok" density="compact" hide-details />
                            <v-checkbox label="Expired" v-model="formData.isExpired" density="compact" hide-details />
                        </v-col>
                    </v-row>
                </v-card-text>
            </v-card>
    ...
    <div class="right-column">
      <v-card class="desktop-form-section flex-grow-1" flat>
        <v-card-title class="text-subtitle-1">Klasifikasi & Lookup</v-card-title>
        <v-card-text>
                    <v-row dense>
                        <!-- Divisi (brg_divisi) -->
                        <v-col cols="5">
                             <v-text-field label="Divisi" v-model="formData.divisiKode" 
                                @click="openDivisiSearch" append-inner-icon="mdi-magnify"
                                variant="outlined" density="compact" hide-details style="cursor: pointer;" />
                        </v-col>
                        <v-col cols="7">
                             <v-text-field v-model="formData.divisiNama" readonly filled density="compact" hide-details />
                        </v-col>

                        <!-- Tipe / Kategori (brg_ktg_kode) -->
                        <v-col cols="5">
                             <v-text-field label="Tipe/Kategori" v-model="formData.tipeKode" 
                                @click="openTipeSearch" append-inner-icon="mdi-magnify"
                                variant="outlined" density="compact" hide-details style="cursor: pointer;" />
                        </v-col>
                        <v-col cols="7">
                             <v-text-field v-model="formData.tipeNama" readonly filled density="compact" hide-details />
                        </v-col>

                        <!-- Jenis (brg_jenis) -->
                        <v-col cols="5">
                             <v-text-field label="Jenis" v-model="formData.jenisKode" 
                                @click="openJenisSearch" append-inner-icon="mdi-magnify"
                                variant="outlined" density="compact" hide-details style="cursor: pointer;" />
                        </v-col>
                        <v-col cols="7">
                             <v-text-field v-model="formData.jenisNama" readonly filled density="compact" hide-details />
                        </v-col>

                        <!-- Gudang Default (brg_gdg_default) -->
                        <v-col cols="5">
                             <v-text-field label="Gudang Default" v-model="formData.gudangKode" 
                                @click="openGudangSearch" append-inner-icon="mdi-magnify"
                                variant="outlined" density="compact" hide-details style="cursor: pointer;" />
                        </v-col>
                        <v-col cols="7">
                             <v-text-field v-model="formData.gudangNama" readonly filled density="compact" hide-details />
                        </v-col>

                        <!-- Supplier Default (brg_sup_kode) -->
                        <v-col cols="5">
                             <v-text-field label="Supplier Default" v-model="formData.supplierKode" 
                                @click="openSupplierSearch" append-inner-icon="mdi-magnify"
                                variant="outlined" density="compact" hide-details style="cursor: pointer;" />
                        </v-col>
                        <v-col cols="7">
                             <v-text-field v-model="formData.supplierNama" readonly filled density="compact" hide-details />
                        </v-col>
                        
                        <!-- Status Radio Group (brg_status) -->
                        <v-col cols="12">
                            <v-radio-group v-model="formData.status" label="Status Barang" inline density="compact" hide-details>
...
const API_BASE_URL = 'http://localhost:8000/api/mmt/bahan'; 
...
const formData = reactive<MasterBarang>({
  ...
  gramasi: '', panjang: 0, lebar: 0,
  gudangKode: 'WH-16', supplierKode: '', jenisKode: 'BB', tipeKode: 'BU', divisiKode: '5',
  isAktif: true, isStok: true, isExpired: false,
  status: 'S', // Default 'S' (Semi Finished) sesuai RadioGroup Delphi
    gudangNama: '', supplierNama: '', jenisNama: '', tipeNama: '', divisiNama: '',
});


// --- Computed ---
const formTitle = computed(() => isEditMode.value ? `Ubah Barang: ${formData.kode}` : 'Master Barang Baru');

const isFormValid = computed(() => {
    // Validasi sederhana
    return formData.kode.length >= 3 && formData.nama.length > 0 && !!formData.gudangKode;
});
...
const closeLookupModal = () => { isLookupVisible.value = false; };
...
const openSupplierSearch = () => openLookup('/api/lookup/supplier', 'supplierKode'); // Sesuai edtsupkodeClickBtn
