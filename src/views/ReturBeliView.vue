...
const api = axios;
const API_BASE_URL = 'http://localhost:8000/api/mmt/returbeli'; 
...
const fetchData = async () => {
 ...
 try {
  ...
  const response = await axios.get<ReturBeliHeader[]>(`${API_BASE_URL}/browse-master`, {
   params: {
...
const loadDetails = async (newlyExpandedItems: ReturBeliHeader[]) => {
 ...
 try {
  ...
  const res = await axios.get<ReturBeliDetail[]>(`${API_BASE_URL}/browse-detail`, {
        params: { nomor: itemToLoad.Nomor, startDate: filters.startDate, endDate: filters.endDate }
    });
...
const handleEdit = () => {
 ...
 router.push({ name: 'ReturBeliEdit', params: { nomor: selectedNomor.value } });
};
